@extends('layouts.app')
@section('title', 'LARAPP - All Categories')
...
@section('content')
    ...
    <a href="{{ route('categories.create') }}" class="bg-green-700 text-white p-4 rounded-md my-10 mx-auto flex justify-center items-center gap-2 w-96 hover:bg-green-600 hover:scale-105 transition ">
    ...
    <div class="p-4">
        ...
        <div class="relative mt-1 flex gap-2 items-center">
            ...
            <input type="text" id="table-search" class="bg-gray-50 border border-gray-300 text-sm rounded-lg block w-80 pl-10 p-2.5" placeholder="Search for categories">
        </div>
    </div>
    ...
    <table class="table bg-white/60 w-full text-sm text-left text-[#614883]">
        <thead class="text-xs text-white uppercase bg-[#614883]">
            <tr>
                ...
                <th class="px-6 py-3 lg:table-cell hidden">DESCRIPTION</th>
        ...
        <tbody id="content">
            @foreach ($categories as $category)
                <tr class="even:bg-white/40">
                    ...
                    <td class="px-6 py-3">
                        <img src="{{ asset($category->image) }}" class="w-8 rounded-full">
                    </td>
                    ...
                    <td class="px-6 py-3 flex items-center justify-center gap-1">
                        <a href="{{ url('categories/'.$category->id) }}" class="w-6 h-6 bg-[#614883] flex justify-center items-center text-white rounded hover:scale-105 transition">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
                            </svg>
                        </a>
                        <a href="{{ url('categories/'.$category->id.'/edit') }}" class="w-6 h-6 bg-[#614883] flex justify-center items-center text-white rounded hover:scale-105 transition">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
