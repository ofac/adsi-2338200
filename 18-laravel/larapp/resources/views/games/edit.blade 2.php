@extends('layouts.app')
@section('title', 'LARAPP - Edit Game')
        ...
        <div class="w-full">
            ...
            <nav class="flex items-center justify-center py-4" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3 text-[#614883]">
                    ...
                    <div class="flex items-center">
                        <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                        <a href="#" class="ml-1 text-sm font-medium">Edit Game</a>
                    </div>
                    </li>
                </ol>
            </nav>
            <section class="flex flex-col break-words bg-white/80 sm:border-1 sm:rounded-md sm:shadow-sm sm:shadow-lg m-4">
                ...
                <form class="w-full px-6 text-[#614883] space-y-6 sm:px-10 sm:space-y-8" method="POST"
                    action="{{ url('games/'.$game->id) }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="flex flex-wrap">
                        ...
                        <input id="name" type="text" class="form-input w-full @error('name')  border-red-500 @enderror border-2 border-[#614883]/40"
                            name="name" value="{{ old('name', $game->name) }}" required autocomplete="name" autofocus>
                    ...
                    <div class="flex flex-wrap">
                        ...
                        <textarea id="description" class="form-input w-full @error('description')  border-red-500 @enderror border-2 border-[#614883]/40"
                            name="description" required>{{ old('description', $game->description) }}</textarea>
                    ...
                    {{--  --}}
                    <div class="flex flex-wrap">
                        <label for="category_id" class="block text-sm font-bold mb-2 sm:mb-4">
                            Category:
                        </label>
                        
                        <select name="category_id" id="category_id" class="form-select w-full @error('category_id') border-red-500 @enderror border-2 border-[#614883]/40" required>
                            <option value="" @if(!old('category_id')) selected @endif disabled> Select Category...</option>
                    ...
                    {{--  --}}
                    <div class="flex flex-wrap">
                        <label for="slider" class="block text-sm font-bold mb-2 sm:mb-4">
                            Slider:
                        </label>
                        
                        <select name="slider" id="slider" class="form-select w-full @error('slider') border-red-500 @enderror border-2 border-[#614883]/40" required>
                            <option value="" @if(!old('slider')) selected @endif disabled> Select Important Game...</option>
                            <option value="0" @if(old('slider', $game->slider) == 0) selected @endif>No</option>
                            <option value="1" @if(old('slider', $game->slider) == 1) selected @endif>Yes</option>
                        </select>
                    ...
                    {{--  --}}
                    <div class="flex flex-wrap">
                        <label for="price" class="block text-sm font-bold mb-2 sm:mb-4">
                            Price:
                        </label>

                        <input id="price" type="number" class="form-input w-full @error('price')  border-red-500 @enderror border-2 border-[#614883]/40"
                            name="price" value="{{ old('price', $game->price) }}" required>
